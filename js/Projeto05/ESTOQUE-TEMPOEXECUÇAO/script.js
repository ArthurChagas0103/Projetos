...
btnEnviar.onclick = function () {
    ...
    if (produto.nome != "" && produto.quantidade != "" && produto.preco != "" && produto.prateleira != "" && produto.descricao != "" && produto.categoria) {
        BD.push(produto);
        tabela.innerHTML += `<tr><td><input type="checkbox" id="${produto.id}" onchange="verificar(this.id)"></td><td>${produto.nome}</td><td>${produto.quantidade}</td><td>${produto.preco}</td><td>${produto.prateleira}</td><td>${produto.descricao}</td><td>${produto.categoria}</td></tr>`;
        limpar();
    }
    else {
